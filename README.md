...
**https://video-analyze.nas.haiqingd.top:1443** （生成攻略需要访问密钥；浏览已生成的攻略无需密钥）

| 首页 · 左侧栏按合集浏览 | 合集内选集 + 攻略正文 | 关键画面（点击跳转 B 站对应时间点） |
|---|---|---|
| ![首页：合集列表](docs/demo-home.png) | ![攻略页：合集选集](docs/demo-guide.png) | ![关键画面](docs/demo-shots.png) |

> **版权说明**：Demo 站点中展示的视频封面、关键画面截图与攻略内容，均源自 B 站 UP 主（如 脆音音、深辰S 等）的原创视频，版权归**原视频作者**所有；截图与文字版攻略仅为个人学习与功能演示用途，请勿二次传播或商用。
...
- **合集（选集）**：已生成的攻略按 B 站合集自动归档；左侧栏两级浏览（合集列表 → 合集内视频，可返回），攻略页支持「上一集 / 下一集」（键盘 ← / →）；提交合集内的视频时会弹出选集面板，已解析的集数带标记，可勾选多集批量生成
...
- **关键画面截图**：视频流仅供截帧、用完即删（不留磁盘），OpenCV 按攻略时间点截帧，失败自动降级为无图文档
